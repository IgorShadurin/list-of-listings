...
const categoryEntries = new Map(categories.map((category) => [
  category,
  listings.filter((entry) => entry.category === category)
]));

function categoryFile(category) {
  return `${slugify(category)}.md`;
}

function categoryLink(category) {
  return `categories/${categoryFile(category)}`;
}

function categoryPage(category, entries) {
  return [
    "<!-- This file is generated by scripts/build-readme.mjs. Edit data or scripts, then run npm run build. -->",
    "",
    `# ${markdownEscape(category)}`,
    "",
    `**${formatNumber(entries.length)}** publication and discovery listings for this category.`,
    "",
    `[← Browse all categories](../README.md#categories) · [Structured JSON](../data/listings.json) · [Contribute](../CONTRIBUTING.md)`,
    "",
    table(entries),
    "",
    `_Catalog snapshot: ${catalog.catalog_date}. Generated from schema version ${catalog.schema_version}._`,
    ""
  ].join("\n");
}
...
lines.push(`[![GitHub lists](https://img.shields.io/badge/GitHub_lists-${stats.by_platform.github ?? 0}-181717?style=for-the-badge&logo=github)](#categories)`);
lines.push(`[![Web channels](https://img.shields.io/badge/web_channels-${stats.by_platform.web ?? 0}-0ea5e9?style=for-the-badge)](#categories)`);
...
lines.push("");
lines.push(`[Browse categories](#categories) · [Browse the JSON](data/listings.json) · [Search from the terminal](#search-and-automate) · [Connect through MCP](#mcp-server) · [Contribute](CONTRIBUTING.md)`);
lines.push("");
lines.push("</div>");
lines.push("");
...
lines.push("");
lines.push(`Browse all **${formatNumber(stats.total)}** entries by category. Each link opens a separate generated page, so the root README stays easy to scan while every category retains its complete listing table.`);
lines.push("");
lines.push("- [List of Listings](#list-of-listings)");
for (const category of categories) {
  const entries = categoryEntries.get(category);
  lines.push(`  - [${markdownEscape(category)}](${categoryLink(category)}) — ${formatNumber(entries.length)} listings`);
}
...
lines.push("");
lines.push(`Browse the complete [MCP & Agent Publishing category](${categoryLink("MCP & Agent Publishing")}) for the documented targets and their submission methods.`);
lines.push("");
...
await Promise.all(categories.map((category) =>
  writeFile(resolve(ROOT, "categories", categoryFile(category)), categoryPage(category, categoryEntries.get(category)))
));
console.log(`Built README.md (${Buffer.byteLength(output).toLocaleString("en-US")} bytes) and ${categories.length} category pages.`);
